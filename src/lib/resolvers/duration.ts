 ...
 * Mimics Skyra's duration resolver.
 */
 ...
 */
export function resolveDurationDate(parameter: string): Result<Date, UserError> {
	return resolveDuration(parameter).map((offset) => new Date(Date.now() + offset));
}
