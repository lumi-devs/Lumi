...
export default class AfkService extends Service {
  public async setAfk(
    ...
    await setAfkCooldown(
      RedisKeys.afkRemovalCooldown(guildId, user.id),
      AFK_REMOVAL_COOLDOWN_MS,
    );
  ...
  public async cleanStaleEntries() {
    ...
    for (const entry of entries) {
      const shardCount = (this.container.client.ws as any).getShardCount() ?? 1;
      const shardId = Number(
        (BigInt(entry.guildId) >> 22n) % BigInt(shardCount),
      );
      if (!(this.container.client.ws as any).getShardIds().includes(shardId))
        continue;
