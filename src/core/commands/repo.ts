...
export class RepoCommand extends EmberSubcommand {
  ...
  public async messageRunAdd(message: Message, args: Args): Promise<void> {
    ...
    const branch = await args.pick("string").catch(() => "master");
  ...
  public async messageRunList(message: Message): Promise<void> {
    ...
    await message.reply(makeListCard(t, "Added Repositories", list));
  }
  ...
  public async messageRunModules(message: Message, args: Args): Promise<void> {
    ...
    try {
      ...
      const t = await fetchT(message);
      await message.reply(makeListCard(t, `Modules in ${repoName}`, list));
    } catch (err: unknown) {
      const error = err as Error;
      await message.reply(
