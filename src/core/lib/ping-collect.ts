...
async function getGatewayNode() {
  try {
    const res = await fetch("https://discord.com/cdn-cgi/trace").then((r) =>
      r.text(),
    );
...
async function probePrisma() {
  ...
  await raw.$queryRawUnsafe("SELECT 1");
  return Date.now() - t;
}
...
async function postgresStats() {
  ...
  try {
    const [[ov], tables, [tx]] = await Promise.all([
      raw.$queryRawUnsafe<{ size: string; uptime_secs: string }[]>(
        "SELECT pg_size_pretty(pg_database_size(current_database())) AS size, extract(epoch from (now() - pg_postmaster_start_time()))::int::text AS uptime_secs",
      ),
      raw.$queryRawUnsafe<{ relname: string; bytes: string; dead: string }[]>(
        "SELECT relname, pg_total_relation_size(relid)::text AS bytes, n_dead_tup::text AS dead FROM pg_stat_user_tables ORDER BY pg_total_relation_size(relid) DESC LIMIT 6",
      ),
      raw.$queryRawUnsafe<{ commits: string; rollbacks: string }[]>(
        "SELECT xact_commit::text AS commits, xact_rollback::text AS rollbacks FROM pg_stat_database WHERE datname = current_database()",
      ),
    ]);
