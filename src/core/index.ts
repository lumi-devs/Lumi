...
export class CoreModule extends Module {
	public constructor(context: Piece.LoaderContext, options: Piece.Options = {}) {
		...
		// Register core piece paths so that Sapphire stores load core commands, listeners, interaction-handlers, and preconditions
		container.stores.registerPath(new URL('./', import.meta.url));
		container.stores.registerPath(new URL('./permissions/', import.meta.url));
		container.stores.get('listeners').registerPath(new URL('./sentry/', import.meta.url));
	}

	public override onLoad() {
		container.logger.info('[Core] Initializing Downloader RPC handlers...');
