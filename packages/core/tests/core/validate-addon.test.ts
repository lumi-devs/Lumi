...
async function makeAddon(
  ...
  // Every addon needs a manifest now — discovery reads it instead of importing
  // the addon's index. Tests that assert on the manifest supply their own.
...
describe("validateAddon", () => {
  ...
  it.each(["tasks", "scheduled-tasks"])(
    "flags a %s/ directory, which a sandboxed addon cannot own",
    async (dirName) => {
      const dir = await makeAddon(
        `trap-${dirName}`,
        { "info.json": JSON.stringify({ name: `trap-${dirName}`, author: ["T"], description: "d", short: "s", version: "1.0.0", end_user_data_statement: "d" }), "index.ts": GOOD_INDEX },
        [dirName],
      );
      const { errors } = await validateAddon(dir);
      expect(errors.some((e) => e.includes("registerTaskFireHandler"))).toBe(true);
    },
  );
  ...
  it("flags any use of Sapphire's container, which does not exist in an addon process", async () => {
