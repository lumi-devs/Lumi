...
/**
 * Static, import-free structural validation for an addon directory. Shared by
 * the `bun run validate` CLI (CI gate) and `DownloaderService.installModule`
 * (pre-load gate) so a broken addon is rejected before its code is ever loaded.
 *
 * Everything here is filesystem + source-text analysis — it never imports the
 * addon, so it is safe to run without the bot's runtime env.
 */
...
const infoSchema = z.object({
  ...
  author: z.array(z.string().min(1)).min(1, "must list at least one author"),
...
export async function validateAddon(dir: string): Promise<ValidationResult> {
    ...
    try {
      ...
      if (!parsed.success) {
        for (const issue of parsed.error.issues)
          errors.push(
            `info.json: ${issue.path.join(".") || "(root)"} — ${issue.message}`,
          );
      } else if (parsed.data.name !== base) {
        errors.push(
          `info.json "name" (${parsed.data.name}) must match the directory name (${base}).`,
        );
      }
    } catch (err) {
      errors.push(
  ...
  // ── source scan: cross-module imports + banned patterns ──────────────────
...
/**
 * Validate one addon or a directory of addons (repo root with per-addon
 * subdirectories). Keyed by addon name.
 */
