...
/**
 * Guild settings + per-guild module config (the `Guild` and `GuildModuleConfig`
 * tables).  Owns the `guildSettings` / `guildConfig` Redis keys.
 */
export class ConfigRepository extends Repository {
  ...
  public async getGuildSettings(guildId: string): Promise<Readonly<Guild>> {
  ...
  public async getAllModuleConfig(
    guildId: string,
    moduleName: string,
