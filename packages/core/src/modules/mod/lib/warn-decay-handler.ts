...
export async function handleWarnDecayFire(_payload: WarnDecayPayload): Promise<void> {
  ...
  for (const c of cases) {
    if (!guildConfigs.has(c.guildId)) {
      const config = await container.db.config.getModuleConfig(c.guildId, "mod", "warn_decay_days");
    ...
    if (diffDays > decayDays) {
      ...
      container.logger.debug(`[WarnDecay] Decayed warn case #${c.caseNumber} for user ${c.userId} in guild ${c.guildId}`);
    }
  }
}
