...
export function resolveCommandError(
  ...
  // Plain string thrown — treat as a message verbatim.
  if (typeof error === "string") {
    return { title: "Command Error", message: error, expected: true };
  }

  // Sapphire wraps thrown values from Result-returning paths; unwrap and recurse.
  if (error instanceof ResultError) {
    return resolveCommandError(label, error.value);
  }

  // Expected, user-facing errors raised by preconditions/commands.
  ...
  // Operational Discord REST failure — log for visibility but don't alarm.
  ...
  // Genuine unhandled error — log loudly and capture for triage.
