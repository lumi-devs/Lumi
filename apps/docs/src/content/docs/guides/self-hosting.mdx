...
Provisions native Postgres and Redis binaries and drops you into a shell with
both running - no Docker daemon required. Point `POSTGRES_URL` /
`DIRECT_POSTGRES_URL` and the Redis variables in `.env` at the addresses it
prints.

</TabItem>
<TabItem label="Manual" icon="pencil">

Install Postgres 17 and Redis 7 yourself (native packages, a VM, whatever you
already run), then point `.env` at them:
...
Lumi itself is just a Bun process reading connection strings - nothing in the
code requires Docker or Nix.
...
Run the backing services (Postgres, Redis) as their own systemd/Docker units, and point `.env` at them the same way you would locally. If you're running the [dashboard](#6-optional-the-web-dashboard) on bare metal too, give it a matching unit with `ExecStart=/usr/bin/env bun run --cwd apps/dashboard start` (after `build`).
