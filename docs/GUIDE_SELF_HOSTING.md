...
If you're running `bun run --cwd apps/worker start` directly on a server rather than through Docker Compose (which already restarts on failure via `restart: unless-stopped`), keep it alive across reboots and crashes with a systemd unit:
...
Run the backing services (Postgres, Redis) as their own systemd/Docker units, and point `.env` at them the same way you would locally. If you're running the dashboard (§ 6) on bare metal too, give it a matching unit with `ExecStart=/usr/bin/env bun run --cwd apps/dashboard start` (after `build`).
