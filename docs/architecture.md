...
- **Persistent top-nav + content-pane via layout route group.** YAGPDB's control panel splits a persistent per-guild nav (`cp_nav.html`) from a swappable content pane rebuilt on every request. Next's `app/guild/[guildId]/layout.tsx` renders the top nav (`GuildTopNav`) once per navigation instead of on every page.
